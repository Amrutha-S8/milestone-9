...
class BackupService:
    ...
    def _backup_database(self, backup_path: Path) -> list[str]:
        ...
        for db_path in ["review_data/reviews.db", "test_reviews.db", "stayza.db"]:
            p = Path(db_path)
            if p.exists():
                shutil.copy2(db_path, backup_path / p.name)
                files.append(p.name)
        return files
    ...
    def cleanup_old_backups(self, max_backups: int | None = None):
...
class RestoreService:
    ...
    def restore(self, backup_name: str, target_dir: Path | None = None) -> Path:
        ...
        if backup_name.endswith(".tar.gz"):
            ...
            msg = f"Backup not found: {backup_name}"
