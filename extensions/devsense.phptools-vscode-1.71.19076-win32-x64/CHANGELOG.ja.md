## 1.71.19076 (2026年7月16日)


### コードアクション

新しく、エディターは`#[Deprecated( ..., replacement: ... )]`属性で注釈された非推奨のシンボルに対するクイックフィックスを提供します。

### 小さな改善

- `@property`定義は、`private`として宣言されている同じプロパティがある場合に尊重されます。 [#1055](https://github.com/DEVSENSE/phptools-docs/issues/1055)
- PHPStanのPHPDocタイプヒントにおける配列オフセットの構文、すなわち`TArray['key']`が認識され尊重されます。ジェネリックの場合の型推論はまだ制限されていますが。 [#632](https://github.com/DEVSENSE/phptools-docs/issues/632)
- 関数呼び出しの間接的な引数検証とパラメータのインレイヒント。
- `instanceof`式内での未知のクラス名に対するクイックフィックス。
- コード補完に一般的に使用されるスニペットを追加しました。
- インターフェースを実装するコードアクション（_抽象メンバーを実装_）はプロパティも実装します。
...
- 有効なコールバック関数のチェックでは、関数名だけでなく `string` でもあることを考慮します。[#1053](https://github.com/DEVSENSE/phptools-docs/issues/1053)
- プロパティの設定の可視性をチェックします。[#2589](https://community.devsense.com/d/2589)
- `__construct` への参照を持つコードレンズは `new` の参照もカウントします（_すべての参照を検索_ と一貫した動作）。[#1050-comment](https://github.com/DEVSENSE/phptools-docs/issues/1050#issuecomment-4742244665)
- 改行文字がエスケープされた HEREDOC 文字列内の誤警告を修正しました。[#2577](https://community.devsense.com/d/2577), [#1053](https://github.com/DEVSENSE/phptools-docs/issues/1053), [#2591](https://community.devsense.com/d/2591)。この更新により、HEREDOC テキスト内の正確なエラーレポートも追加され、不正なインデントのある行が下線で示されます。
...
- 配列型コーラブル構文でメソッド名のコード補完が追加されました。
- 文字列リテラルへの_すべての参照を検索_を使用すると、ユーザーのコード内でそのテキストがすべてリストされます。
- 既知の文字列値（例：WordPressフック名）への_すべての参照を検索_。[#2588](https://community.devsense.com/d/2588)
- 配列型コーラブル構文内の関数のツールチップ。
- 文字列型および配列型コーラブルでの関数名の検証とナビゲーション。
  ...
  **配列型コールバック**と**文字列型コールバック**は検証され、コンテキストに応じた補完、ナビゲーション、セマンティックハイライト、リファクタリングの名前変更、および参照された関数の詳細を含むツールチップを取得します。

### WordPress

- _WordPress_スタブの関数とクラスには、ドキュメントへの正しいリンクがあります。[#1039](https://github.com/DEVSENSE/phptools-docs/issues/1039)
- WordPressフック呼び出しへの_ナビゲーション_。
- WordPressフックへの_すべての参照を検索_。
- ツールチップは、文字列値の型を示します（例：`(WordPress Hook) 'wp_head'`）。
...
PHPマニュアルの多言語版全体が更新され、新しいPHPスタブ、追加の翻訳、およびこれまで文書化されていなかったPHP関数やクラスが追加されました。これらの更新により、コードインテリジェンス、コード解析、コードアクションが向上します。
...
- コードアクションが無効な場合、リファクタリングヒントを表示しません。[#1038](https://github.com/DEVSENSE/phptools-docs/issues/1038)
- `is_null()`コードアクションは、必要に応じて括弧を追加します。
- _Laravel_: グループ宣言内の`Route`定義からコントローラーのアクションへのナビゲーションを改善します。
- 初期化されていない変数を返す関数は`mixed`を返すものとして扱われます。[#1045](https://github.com/DEVSENSE/phptools-docs/issues/1045)
- ラムダの`use()`で未知のグローバル変数は報告されません。[#1044](https://github.com/DEVSENSE/phptools-docs/issues/1044)
- 仮の型を尊重し、クラスオーバーライドでの誤警告を修正します。[#1043](https://github.com/DEVSENSE/phptools-docs/issues/1043)
- `BcMath`のサポートを改善します。[#2559](https://community.devsense.com/d/2559)
- _Outline_ビュー内での`namespace`シンボルは、`"outline.showNamespaces"`設定に従って表示されます。[#2578](https://community.devsense.com/d/2578)
...
- `BcMath\Number`によってオーバーライドされた算術演算子を型推論が尊重します。[#2559](https://community.devsense.com/d/2559)
- エラーコードなしで単独の`@disregard`タグ（`@suppress`のエイリアス）を尊重します。ステートメントや関数の上にあるタグは、エディターがすべての警告を無視するようにします。
- ジェネリックテンプレート型で注釈された変数の後のインテリセンスが向上しました。[#2565](https://community.devsense.com/d/2565)
- 配列アクセスおよびforeachループ内の型推論の曖昧性を尊重します。[#2522](https://community.devsense.com/d/2522)
- 未使用の`$_`に対応する警告はデフォルトで無視されます。なぜなら、`$_`はダミー変数と見なされるためです。[#2570/2](https://community.devsense.com/d/2570-at-suppress-is-not-working-may-2026/2)
- `@require-extends`のチェックを修正しました。[#2572](https://community.devsense.com/d/2572)

### UI

- `"hostname"` `launch.json`プロパティがVSCodeによって認識されます。[#2551/3](https://community.devsense.com/d/2551/3)
- VSCodeの表示言語が中国語の場合、新しいチェンジログが中国語で表示されます。
...
- `"numeric-string"` 型が遵守されるようになり、数値文字列からの暗黙的変換で不要な警告が出なくなりました。
- さまざまな制御フロー分析の整理と安定性の向上。

### PHPDoc

- ネストしたPHPDocタグの解析がより寛容になり、PHPDoc解析の堅牢性が向上しました。
- `@global` の出現箇所のマッチングを修正しました。
- PHPDocタグでの型名コード補完を修正しました。[#892 (comment)](https://github.com/DEVSENSE/phptools-docs/issues/892#issuecomment-4422799460)

### IntelliSense

- `$GLOBALS['NAME']` がマウスホバーやリファクタリング機能でより具体的に処理されるようになりました。
- 配列に複数の要素型がある場合の動的プロパティの型推論を修正しました。

### WordPress

- より多くの非標準的なWordPress型構文との互換性を追加しました。
- 構造化されたWordPress型構文の習慣への対応を拡張しました。
