...
from datasets.loader import BenchmarkItem, DatasetLoader
...
class IntentAccuracyEvaluator:
    ...
    def evaluate(self, items: List[BenchmarkItem]) -> Dict[str, Any]:
        total = len(items)
        if total == 0:
    ...
    def evaluate_language(self, items: List[BenchmarkItem], language: str) -> Dict[str, Any]:
