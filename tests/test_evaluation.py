...
from evaluation.completion import FlowCompletionEvaluator, FLOW_SCENARIOS
...
class TestLatencyProfiler:
    ...
    def test_measure_language(self):
        ...
        res, timing = profiler.measure_language(fast_func, "English")
        assert res == 42
...
class TestIntentAccuracyEvaluator:
    ...
    def test_per_language_breakdown(self):
        ...
        assert "per_language" in results
        for lang, data in results["per_language"].items():
...
class TestEvaluationEngine:
    ...
    def test_per_language_scores(self, engine):
        ...
        for lang, score_data in scores.items():
...
class TestEvaluationAPI:
    ...
    def test_languages_status_endpoint(self):
        ...
        data = response.json()
        assert len(data) > 0
        for lang, status_data in data.items():
    ...
    def test_evaluation_results_before_run(self):
        from evaluation.engine import EvaluationEngine
        from api.dependencies import get_evaluation_engine
        response = self.client.get("/language/evaluation/results")
        assert response.status_code == 200
